...
const CartItems = () => {
  ...
  const { all_products, cartItems, removeFromCart, TotalCartAmount, addOne, reduceOne} = useContext(ShopContext);

  return (
    <section className='max-pad-container bg-primary rounded-3xl'>
      <div className='py-10'>
              ...
              if(cartItems[e.id]>0){
                return  <tr key={e.id} className='border-b border-slate-900/20 text-gray-30 p-6 medium-14 text-center'>
                          <td className='flex items-end justify-center'>
                            <img src={e.image} alt="productImage" height={55} width={55} className='rounded-lg ring-1 ring-slate-900/5 m-3 p-1'/>
                          </td>
                          <td>
                          ...
                          <div className='flex flex-row justify-evenly text-center line-clamp-3'>
                            <FaPlus onClick={() =>
                                {addOne(e.id)}
                              } className='my-1'/>
                            {cartItems[e.id]}
          ...
          <div className='flex flex-col gap-8'>
            ...
            <div>
              ...
              <div className='flexBetween py-4'>
                ...
                <h4 className='bold-18'>${TotalCartAmount()}</h4>
              </div>
            </div>
            <button className='btn-dark w-44 rounded-xl'>Checkout</button>
          </div>
          <div className='flex flex-col gap-10'>
            <h4 className='bold-20 flex flex-row'>
            Enter your coupon code here: <FaQuestionCircle className="fa fa-question-circle"/><span className="tip-txt rounded-3xl relative">The coupon code should look like this: HNY2025</span>
            </h4>
